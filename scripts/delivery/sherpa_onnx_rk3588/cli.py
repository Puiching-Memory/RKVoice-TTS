...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Prepare, assemble, and deploy a sherpa-onnx RK3588 ASR+TTS runtime bundle")
    ...
    def add_remote_args(subparser: argparse.ArgumentParser) -> None:
        ...
        subparser.add_argument("--skip-rknn-smoketest", action="store_true", help="Run remote smoke test without the RKNN ASR verification step")

    download_parser = subparsers.add_parser("download", help="Prepare the local sherpa-onnx source bundle")
    add_source_args(download_parser)

    build_parser = subparsers.add_parser("build", help="Assemble the local sherpa-onnx runtime bundle")
...
def main() -> None:
    ...
    stage_dir = resolve_path_option(
        ...
        env_names=("RKVOICE_STAGE_DIR", "TTS_STAGE_DIR"),
    ...
    if args.action in {"upload", "all"}:
        ...
        remote_dir = resolve_text_option(
            args.remote_dir,
            env_names=("RKVOICE_REMOTE_DIR", "TTS_REMOTE_DIR"),
            local_settings=local_settings,
        ...
        text = resolve_text_option(
            args.text,
            env_names=("RKVOICE_TTS_TEXT", "RKVOICE_SENTENCE", "TTS_SMOKE_TEST_SENTENCE", "TTS_SENTENCE"),
            local_settings=local_settings,
        ...
        deploy_runtime_bundle(
            ...
            enable_rknn_smoketest=not args.skip_rknn_smoketest,
        )
